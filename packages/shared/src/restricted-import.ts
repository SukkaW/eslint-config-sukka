...
export const BETTER_ALTERNATIVES = Object.entries({
  ...
  classnames: npm('clsx'),
  classcat: npm('clsx'),
  'p-limit': `${npm('@henrygd/queue')} / ${npm('superlock')} (withLock(n)) / ${npm('@livekit/mutex')} (new MultiMutex(n))`,
  'p-mutex': npm('@livekit/mutex') + ' / ' + npm('superlock'),
  ...
  fnv1a: `${npm('fast-fnv1a')} which is 6x faster`,
  '@sindresorhus/fnv1a': `${npm('fast-fnv1a')} which is 6x faster, or ${npm('fnv1a52')} if you need more hash space, or ${npm('fnv1a-64')} if you really need BigInt`,
